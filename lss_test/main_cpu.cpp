...
{
  uint32_t i = 0, size = 32;
  uint32_t cls[5];
  float prob[5];

  float* output_data = get_data_from_file("input_img.bin_output0_1_32.txt", 32);
...
int main()
{
  ...
  std::cout << " ********** run lss test_model **********" << std::endl;
  system("./hhb_out_cpu/hhb_runtime ./hhb_out_cpu/hhb.bm input_img.bin");
