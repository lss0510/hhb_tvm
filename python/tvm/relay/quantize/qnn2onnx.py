...
class InsertQDQToQNN:
    ...
    def __init__(self) -> None:
        ...
        self.ignored_ops = ["qnn.csi.quantize", "qnn.csi.dequantize"] + self.multi_output_ops
            ...
            def visit_call(self, call):
                ...
                if call.op.name in list(csi_op().conv_handle.keys()) + ["qnn.csi.dense"]:
                    ...
                    if isinstance(bias_data, float) and math.isclose(bias_data, 0.0):
                        ...
                        b_quant_info = qc.get_quant_params(call_attrs["q_params"][2], b_tensor_type)
...
class MatMul(QnnOpConverter):
    ...
    def _impl_v1(
        ...
        qnn_attrs = _qnn_attrs(node_entry["relay_node"].attrs)
        onnx_attrs = cls.convert_attributes(qnn_attrs)
        if qnn_attrs["transpose_a"] == 0 and qnn_attrs["transpose_b"] == 0:
            onnx_node = onnx.helper.make_node(
                cls.__name__,
                [node_entry["input_names"][0], node_entry["input_names"][1]],
                ["temp_name_" + qnn_attrs["layer_name"]],
                qnn_attrs["layer_name"],
            )
            model_container.add_nodes([onnx_node])
            onnx_node = onnx.helper.make_node(
                "Add",
                ["temp_name_" + qnn_attrs["layer_name"], node_entry["input_names"][2]],
                node_entry["output_names"],
                "Add" + "_" + qnn_attrs["layer_name"],
            )
            model_container.add_nodes([onnx_node])
        elif qnn_attrs["transpose_a"] == 1 and qnn_attrs["transpose_b"] == 0:
            onnx_node = onnx.helper.make_node(
                "Transpose",
                [
                    node_entry["input_names"][0],
                ],
                ["temp_a_" + qnn_attrs["layer_name"]],
                "Transpose" + "_" + qnn_attrs["layer_name"],
                **onnx_attrs,
            )
            model_container.add_nodes([onnx_node])
            onnx_node = onnx.helper.make_node(
                cls.__name__,
                ["temp_a_" + qnn_attrs["layer_name"], node_entry["input_names"][1]],
                ["temp_output_" + qnn_attrs["layer_name"]],
                qnn_attrs["layer_name"],
            )
            model_container.add_nodes([onnx_node])
            onnx_node = onnx.helper.make_node(
                "Add",
                ["temp_output_" + qnn_attrs["layer_name"], node_entry["input_names"][2]],
                node_entry["output_names"],
                "Add" + "_" + qnn_attrs["layer_name"],
            )
            model_container.add_nodes([onnx_node])
        elif qnn_attrs["transpose_a"] == 0 and qnn_attrs["transpose_b"] == 1:
            onnx_node = onnx.helper.make_node(
                "Transpose",
                [
                    node_entry["input_names"][1],
                ],
                ["temp_b_" + qnn_attrs["layer_name"]],
                "Transpose" + "_" + qnn_attrs["layer_name"],
                **onnx_attrs,
            )
            model_container.add_nodes([onnx_node])
            onnx_node = onnx.helper.make_node(
                cls.__name__,
                [node_entry["input_names"][0], "temp_b_" + qnn_attrs["layer_name"]],
                ["temp_output_" + qnn_attrs["layer_name"]],
                qnn_attrs["layer_name"],
            )
            model_container.add_nodes([onnx_node])
            ...
            model_container.add_nodes([onnx_node])
        elif qnn_attrs["transpose_a"] and qnn_attrs["transpose_b"]:
            onnx_node = onnx.helper.make_node(
                "Transpose",
                [
                    node_entry["input_names"][0],
                ...
                ["temp_a_" + qnn_attrs["layer_name"]],
                "Transpose_a" + "_" + qnn_attrs["layer_name"],
                **onnx_attrs,
            )
            model_container.add_nodes([onnx_node])
            onnx_node = onnx.helper.make_node(
                ...
                ["temp_b_" + qnn_attrs["layer_name"]],
                "Transpose_b" + "_" + qnn_attrs["layer_name"],
                **onnx_attrs,
            )
            model_container.add_nodes([onnx_node])
            input_names = [
...
class Slice(QnnOpConverter):
    ...
    def _impl_v1(cls, node_entry, model_container, node_dict):
        qnn_attrs = _qnn_attrs(node_entry["relay_node"].attrs)
        name = node_entry["name"]
        begin = numpy.asarray(qnn_attrs["begin"], dtype=numpy.int32)
        end = numpy.asarray(qnn_attrs["end"], dtype=numpy.int32)
        axes = numpy.asarray(range(len(begin)), dtype=numpy.int32)
        steps = numpy.asarray(qnn_attrs["strides"], dtype=numpy.int32)
        input_names = [
            node_entry["input_names"][0],
